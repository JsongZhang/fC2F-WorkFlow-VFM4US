...
def get_arguments():
    ...
    parser.add_argument('-pretrained', type=str, default='/SZU_DATA/us-vfm/dinov2_output_us120k_us380kpretrain/checkpoint_124999.pth')
    ...
    if args.pretrained and os.path.isfile(args.pretrained):
        ...
        for k, v in state_dict.items():
            ...
            if k.startswith("student.backbone."):
                ...
                # 修复层级名称，如 blocks.0.0 转 blocks.0
                new_key = re.sub(r"\.blocks\.(\d+)\.(\d+)\.", r".blocks.\1.", new_key)

                # 将转换后的 key 和原始 key 打印出来，便于调试
                print(f"Original key: {k} -> New key: {new_key}")

                # 添加到新的 state_dict 中
                new_state_dict[new_key] = v
            else:
                # 如果没有前缀 student.backbone，直接复制
                new_state_dict[k] = v

        # 加载到模型
        missing_keys, unexpected_keys = net.encoder.load_state_dict(new_state_dict, strict=False)
        ...
        for name, param in net.named_parameters():
            if name in missing_keys:
                print(f"Reinitializing missing layer: {name}")
                if param.requires_grad:
                    torch.nn.init.xavier_uniform_(param)

    else:
        print("=> No pretrained weights found or Pretrained flag is False")
